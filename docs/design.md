...
| Need | Where | Status |
|---|---|---|
| Be woken when asleep (SIP-45) | `WakeReceiver.kt` → `WakeService.kt` → `Native.wake` → `sigil_phone::window` | window proven on a desktop; the JNI is built and exported; **no wake has ever arrived**, for want of a distributor (below). **And none could have** (found 2026-09-22): nothing made the first registration -- the window re-registered an endpoint no exchange had been told, and `wake::forget` had no caller. The session registers now (`Cmd::WakeEndpoint`, on every connect; the phone offers its endpoint through `sigil::wake` at launch, on the distributor's word, and on a ttl change), forgets on unregistration, and both are proven against an exchange that then wakes a loopback distributor. **What that costs, seen** (2026-09-22): with sigil gone from the foreground for half an hour, a call from another exchange was refused *unreachable* -- the account had no event stream open, so there was nothing to ring. A phone without a distributor is a phone that can be called only while sigil is *running*. **And until 2026-09-22 not even then**: with sigil in the background but alive, a ring reached the session and was said to nobody -- everything that posted a notification ran from a frame, and a backgrounded phone draws none. `sigil_chat::announce` now speaks from the session's wake when the frame it asked for does not come (the phone hands the chat app its notifier as the off-frame one); seen on the device: Home pressed, rung from squic.org, the ring on the shade. **And the process kept**: *Stay reachable without one* on the Phone tab (`ReachService`, a `specialUse` foreground service with a quiet notice; `Settings::stay_reachable`, off unless chosen, applied at launch) -- seen on the device: chosen, Home, **ten minutes**, rung from squic.org, the ring on the shade beside the service's notice. Reachable without a distributor, at the battery's cost, by the person's choice |
...
| A distributor the person chooses | UnifiedPush connector; embedded FCM distributor as fallback | **still nothing installed** -- but the Phone tab says what a distributor is, offers two links to get one, and offers the alternative: staying reachable without one (above). The embedded bridge has no gateway, so it no longer offers itself as one |
| A call with the microphone open, visibly | `CallService.kt`, `Notify::calling` | **seen on the phone** (2026-09-22): answering a call from another exchange started `CallService` as a foreground service (`isForeground=true`, types microphone + phoneCall, an ongoing notification in the *quiet* channel, category call), and hanging up stopped it -- read off `dumpsys activity services` during and after the call |
