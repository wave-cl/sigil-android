...
| Need | Where | Status |
|---|---|---|
| Be woken when asleep (SIP-45) | `WakeReceiver.kt` → `WakeService.kt` → `Native.wake` → `sigil_phone::window` | window proven on a desktop; the JNI is built and exported; **no wake has ever arrived**, for want of a distributor (below). **What that costs, seen** (2026-09-22): with sigil gone from the foreground for half an hour, a call from another exchange was refused *unreachable* -- the account had no event stream open, so there was nothing to ring. A phone without a distributor is a phone that can be called only while sigil is *running*. **And until 2026-09-22 not even then**: with sigil in the background but alive, a ring reached the session and was said to nobody -- everything that posted a notification ran from a frame, and a backgrounded phone draws none. `sigil_chat::announce` now speaks from the session's wake when the frame it asked for does not come (the phone hands the chat app its notifier as the off-frame one); seen on the device: Home pressed, rung from squic.org, the ring on the shade |
